...
--
-- The `tmdb_api_key`, `tgdb_api_key`, `hardcover_api_key`, and
...
INSERT INTO plugin_settings (user_id, plugin_id, key, value)
SELECT user_id, 'media', 'tgdb_api_key', to_jsonb(tgdb_api_key)
FROM user_settings
WHERE tgdb_api_key IS NOT NULL
ON CONFLICT (user_id, plugin_id, key) DO NOTHING;
...
ALTER TABLE user_settings DROP COLUMN IF EXISTS tgdb_api_key;
