        ...
        <div className="relative">
          ...
          {monthLoading && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="h-8 w-8 rounded-full border-2 border-indigo-500/70 border-t-transparent animate-spin bg-white/40 dark:bg-gray-900/30" />
            </div>
          )}
        </div>
      </div>
...
export function MoodsTab() {
  ...
  // Year-scoped data
  useEffect(() => {
    moodStats.monthly(USER_ID, year)
      .then((monthly) => {
  ...
  useEffect(() => {
    ...
    if (next.get('tab') !== 'plugin:mood') {
      next.set('tab', 'plugin:mood');
      changed = true;
    }
  ...
  useEffect(() => {
    ...
    Promise.all([
      moodStats.daily(USER_ID, rangeStart, rangeEnd),
      moodStats.countRange(USER_ID, rangeStart, rangeEnd),
