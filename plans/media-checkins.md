...
- Yamtrack import: **import everything** — `tv`/`season` rows create TV show entities (no check-in); `episode` rows become Completed episode check-ins (latest of duplicate `start_date`s, tz=UTC); movies/games/books with Completed/In progress/Dropped become check-ins; Planning/Paused rows create the media entity only. Every row's disposition is shown in the import table.
...
| `media_type = episode` | Group duplicates by `(media_id, season, episode)`; take the **latest** `start_date`; insert Completed episode check-in, `checkin_timezone='UTC'`. `external_event_id = sha1(media_id|source|episode|s|e|start_date)`. |
