...
import { User, Link2, Loader2, Check, AlertCircle, Upload, FileText, Cog, Clock, CheckCircle2, XCircle, Play, Send, Ban, StopCircle, Monitor, Sun, Moon, Bell, BellOff, Download, Smile, Plus, Trash2, RefreshCw, MapPin, RotateCcw, ShieldAlert } from 'lucide-react';
import { settings, importApi, jobs, moodActivities, venueMerges, backupApi } from '../api/client';
...
import type { UserSettings, ImportResult, Job, MoodActivityGroup, MoodActivity, VenueMergeSuggestion, BackupImportResult } from '../types';
...
function StartOverSection() {
  ...
  const canAdvance = firstConfirmation.trim() === 'DELETE MY DATA';
  const canSubmit = secondConfirmation.trim() === 'START OVER';
  ...
  const handleStartOver = async () => {
    ...
    setMessage(null);
    try {
      await backupApi.startOver(firstConfirmation.trim(), secondConfirmation.trim());
      setMessage({ type: 'success', text: 'All data has been deleted. Reloading...' });
  ...
  return (
    <div className="bg-red-50/60 dark:bg-red-900/10 backdrop-blur-xl rounded-2xl border border-red-200/80 dark:border-red-900/50 shadow-sm shadow-black/[0.03] p-6 space-y-4">
      ...
      {step === 2 && (
        <div className="space-y-3">
          <p className="text-xs text-red-800/80 dark:text-red-200/80">
            Step 2 of 2: Type START OVER and submit to delete all data.
          </p>
          <input
            type="text"
